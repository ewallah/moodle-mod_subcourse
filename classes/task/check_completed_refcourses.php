    ...
    public function execute(): void {
        ...
        foreach ($rs as $subcourse) {
            ...
            if (!isset($cache[$subcourse->course])) {
                // Load the students enrolled to it.
                // We do not need data from the previous course any more.
                $coursecontext = context_course::instance($subcourse->course);
                $cache = [
                    $subcourse->course => (object)[
                        'participants' => get_enrolled_users($coursecontext, 'mod/subcourse:begraded', 0, "u.id"),
                    ],
                ];
            }
            $completioncache = \cache::make('core', 'coursecompletion');
            ...
            foreach (array_keys($cache[$subcourse->course]->participants) as $userid) {
                $allow = false;
                $values = $completioncache->get("{$userid}_{$subcourse->course}");
                if ($values && $value = current($values)) {
                    ...
                    unset($coursecompletion);
                }
                if ($allow) {
                    $course = get_course($subcourse->course);
